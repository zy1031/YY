...
<template>
  <div class="models-page">
    ...
    <!-- 当前模型 -->
    <div class="current-model-card" v-if="currentModel">
      <div class="current-badge">当前激活</div>
      <div class="current-info">
        <span class="current-name">{{ currentModel.model_name }}</span>
        <span class="current-meta">{{ currentModel.animal_type_name }} · {{ currentModel.framework }}</span>
      </div>
      <div class="current-thresholds">
        <span>置信度: {{ currentModel.confidence_threshold }}</span>
        <span>IOU: {{ currentModel.iou_threshold }}</span>
      </div>
    </div>
    <div class="current-model-card empty" v-else-if="!loading">
      <span>暂未设置检测模型，请从下方列表切换</span>
    </div>
...
.current-meta { font-size: 12px; color: #718096; }
