...
function renderTrend(data: any[]) {
  ...
  trendChart.setOption({
    ...
    legend: { data: ['检测任务数', '视频/监控异常目标'], top: 0 },
...
function renderBar(data: any[]) {
  ...
  barChart.setOption({
    ...
    legend: { data: ['总任务', '异常', '图片行为识别'], top: 0 },
    grid: { left: 50, right: 20, top: 40, bottom: 40 },
    xAxis: {
      type: 'category',
      data: data.map((d) => d.animal_type),
      axisLabel: { color: '#718096', fontSize: 11 },
      axisLine: { lineStyle: { color: '#e2e8f0' } },
    },
    yAxis: {
    ...
    series: [
      ...
      },
      {
        name: '图片行为识别',
        type: 'bar',
        data: data.map((d) => d.image_tasks || 0),
...
<template>
  <div class="stats-page" v-loading="loading">
    ...
    <div v-if="overview" class="overview-grid">
      <div class="ov-card"><span class="ov-num">{{ overview.total_detections }}</span><span class="ov-label">总任务次数</span></div>
      <div class="ov-card"><span class="ov-num">{{ overview.today_detections }}</span><span class="ov-label">今日任务</span></div>
      <div class="ov-card"><span class="ov-num">{{ overview.image_behavior_tasks || 0 }}</span><span class="ov-label">图片行为识别</span></div>
      <div class="ov-card danger"><span class="ov-num">{{ overview.total_abnormal }}</span><span class="ov-label">视频/监控异常目标</span></div>
      <div class="ov-card warning"><span class="ov-num">{{ overview.total_suspicious }}</span><span class="ov-label">视频/监控可疑目标</span></div>
    </div>

    <div class="charts-grid">
      <div class="chart-card wide">
        <h3 class="chart-title">任务趋势与视频/监控异常（近 {{ selectedDays }} 天）</h3>
        <div ref="trendRef" style="width: 100%; height: 240px"></div>
      </div>

      <div class="chart-card">
        <h3 class="chart-title">视频/监控健康状态分布</h3>
        <div ref="pieRef" style="width: 100%; height: 240px"></div>
      </div>

      <div class="chart-card">
        <h3 class="chart-title">按动物类型统计（含图片行为识别）</h3>
...
.ov-card.danger { background:#fff5f5; }
.ov-card.warning { background:#fffbeb; }
.ov-num { font-size:32px; font-weight:700; color:#2d3447; line-height:1; }
.ov-card.danger .ov-num { color:#f56565; }
.ov-card.warning .ov-num { color:#ed8936; }
